					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div>
									<a href = '${pageContext.request.contextPath}/student/toSave.do' id="addStudent">
...
<![endif]-->
		...
		<script type="text/javascript">
            ...
            function editStudent(rowId) {
                ...
                /*var data = "?id=" + rowId + "&studentName=" + rowData.studentName + "&studentNum=" + rowData.studentNum +
                    "&picture=" + rowData.picture + "&gender=" + rowData.gender + "&birthday=" + rowData.birthday +
                    "&className=" + rowData.className;*/
                window.location.href="${pageContext.request.contextPath}/student/toSave.do?studentId=" + rowId;
            }

            function delStudent(rowId) {
                var flag = confirm("确认删除该学生信息吗?")
				if (flag) {
                    $.post("${pageContext.request.contextPath}/student/delete.do",{id:rowId},function (data) {
                        //判断传过来的flag，1代表删除成功，0代表删除失败
                        if (data.flag == 0) {
                            //alert("22");
                            alert("删除学生失败...");
                        } else if(data.flag == 1){
			...
			jQuery(function($) {
				...
				*/
				...
				jQuery(grid_selector).jqGrid({
					...
					url: "${pageContext.request.contextPath}/student/findAllByPage.do",
					//设置提交的方式
					mtype: "post",
					//使用本地数据显示
					//data: grid_data,
					...
					colModel:[
						...
						{name:'id',hidden: true,index:'id', width:60, sorttype:"int", editable: false},
						{name:'picture',hidden: true,index:'picture', width:60, sorttype:"int", editable: false},
						{name:'studentNum',index:'studentNum',width:100, editable:true,sortable:false},
						{name:'studentName',index:'studentName', width:100,editable: true,editoptions:{size:"20",maxlength:"30"}},
						{name:'gender',index:'gender', width:50, editable: true,edittype:"select",editoptions: {value:'1:男;0:女'},formatter:formatGender},
						{name:'birthday',index:'birthday',width:100, editable:true, sorttype:"date",formatter: formatDate,unformat: pickDate},
						{name:'className',index:'className', width:120, sortable:false,editable: true,edittype: "select",editoptions:
							{value: {'请选择班级': '请选择班级'},dataUrl:'${pageContext.request.contextPath}/grade/findAll.do'}},

						{name:'subjectNum',index:'subjectNum',width:80, editable:false,sortable:false},
						{name:'avg',search:false,index:'avg',width:80, editable:false,sortable:true},
                        {name:'ScoreEntry',search:false,index:'', width:90, fixed:true, sortable:false, resize:false,formatter:scoreEntry
                        },
                        {name:'selectSubject',search:false,index:'', width:80, fixed:true, sortable:false, resize:false,formatter:selectSubjects
                        },
                        {name:'operation',search:false,index:'', width:80, fixed:true, sortable:false, resize:false,
				...
				function formatGender(cellvalue, options, rowObject){
					if (cellvalue == 1 || "1" == cellvalue) {
						return "男";
					} else if (cellvalue == 0 || "0" == cellvalue) {
						return "女";
					}
				};
				...
				function formatDate(cellvalue, options, rowObject) {
				    var cellValue = parseInt(cellvalue);
					var dateFormat = new Date(cellvalue);
					return dateFormat.toLocaleDateString();
                }

                //给分数录入提供一个超链接
                function scoreEntry(cellvalue, options, rowObject) {
                    var studentId = rowObject.id;
                    //alert(rowId + rowObject.className);
                    var tag1 = "<a href = '${pageContext.request.contextPath}/score/toScorePage.do?studentId="+ studentId +"'>"+"<button class='btn btn-minier btn-primary no-radius'><i class='ace-icon fa fa-pencil align-top'></i>分数录入</button></a>";
                ...
                //给选课功能添加一个超链接
                function selectSubjects(cellvalue, options, rowObject) {
                    var rowId = rowObject.id;
                    //alert(rowId);//"<a href = 'index.html'>选课</a>"

                    var tag = "<a href='${pageContext.request.contextPath}/subject/toSelectCourseShow.do?studentId="+rowId+"' class='btn btn-minier btn-primary no-radius'><i class='ace-icon fa fa-pencil-square-o'></i>选课<span class='badge badge-warning badge-left'></span></a>";
                    return tag;
                    <%--/*"<a href='<%=request.getContextPath()%>/Admin/editProvider/" + rowdata.providerId + "' class='ui-icon ui-icon-pencil' ></a>";*/--%>
                };
                ...
                function operations(cellvalue, options, rowObject) {
                    ...
                    var html = "<button class='btn btn-primary btn-minier' onclick='editStudent(" + rowId
						+ ")'><i class='ace-icon fa fa-pencil-square-o'></i></button>" +
						"<button class='btn btn-primary  btn-danger btn-minier' onclick='delStudent(" + rowId
