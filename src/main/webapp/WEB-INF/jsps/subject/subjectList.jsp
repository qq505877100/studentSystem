					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div>
									<a href = '${pageContext.request.contextPath}/subject/toSave.do' id="addSubject">
...
<![endif]-->
		...
		<script type="text/javascript">
            ...
            function editSubject(rowId) {
                ...
                var data = "?id=" + rowId + "&subjectName=" + rowData.subjectName + "&limitStu=" + rowData.limitStu;
                window.location.href="${pageContext.request.contextPath}/subject/toSave.do" + data;

                //$.post("${pageContext.request.contextPath}/subject/toSave.do",data);
                //console.log(rowId);
            }

            function delSubject(rowId) {

                var flag = confirm("确认删除该课程吗?");
                if (flag) {
                    $.post("${pageContext.request.contextPath}/subject/delete.do",{id:rowId},function (data) {
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: "${pageContext.request.contextPath}/subject/findAllByPaging.do",
					//设置提交的方式
					mtype: "post",
					//使用本地数据显示
					//data: grid_data,
					...
					colNames:['id','课程名',"选课人数上限",'选修人数','平均分','操作'],
					colModel:[
						{name:'id',hidden: true,index:'id', width:60, sorttype:"int", editable: false},
						{name:'subjectName',index:'subjectName',editable: true,editoptions:{size:"20",maxlength:"30"}},
                        {name:'limitStu',index:'limitStu', editable:false,sortable:false},
						{name:'totalStus',index:'totalStus', editable:false,sortable:false},
						{name:'subjectAvg',search:false,index:'subjectAvg', editable:false,sortable:true},
                        {name:'operation',search:false,index:'', fixed:true, sortable:false, resize:false,
                ...
                function operations(cellvalue, options, rowObject) {
                    ...
                    var html = "<button class='btn btn-primary btn-minier' onclick='editSubject(" + rowId
						+ ")'><i class='ace-icon fa fa-pencil-square-o'></i></button>&nbsp;&nbsp" +
						"<button class='btn btn-primary  btn-danger btn-minier' onclick='delSubject(" + rowId
