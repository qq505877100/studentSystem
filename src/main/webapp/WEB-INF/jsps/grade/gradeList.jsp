...
<html lang="en">
	...
	<body class="no-skin">
		...
		<div id="navbar" class="navbar navbar-default          ace-save-state" style="height: auto">
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div>
									<a href = '${pageContext.request.contextPath}/grade/toSave.do' id="addGrade">
...
<![endif]-->
		...
		<script type="text/javascript">
            ...
            function editClass(rowId) {
                ...
                var data = "?id=" + rowId + "&className=" + rowData.className;
                window.location.href="${pageContext.request.contextPath}/grade/toSave.do" + data;

                //console.log(rowId);
            }

            function delClass(rowId) {
                //alert(rowId);
				var flag = confirm("确认删除该班级信息吗？");
				if (flag) {
                    $.post("${pageContext.request.contextPath}/grade/delete.do",{id:rowId},function (data) {
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: "${pageContext.request.contextPath}/grade/findAllByPaging.do",
					//设置提交的方式
					mtype: "post",
					//使用本地数据显示
					//data: grid_data,
					...
					colNames:['id','班级','人数','平均分','操作'],
					colModel:[
						{name:'id',hidden: true,index:'id', width:60, sorttype:"int", editable: false},
						{name:'className',index:'className',editable: true,editoptions:{size:"20",maxlength:"30"}},
						{name:'totalStus',index:'totalStus', editable:false,sortable:false},
						{name:'classAvg',search:false,index:'classAvg', editable:false,sortable:true},
                ...
                function operations(cellvalue, options, rowObject) {
                    ...
                    var html = "<button class='btn btn-primary btn-minier' onclick='editClass(" + rowId
						+ ")'><i class='ace-icon fa fa-pencil-square-o'></i></button>&nbsp;&nbsp" +
						"<button class='btn btn-primary  btn-danger btn-minier' onclick='delClass(" + rowId
